...
public class EmpleadoDaoImpl
    ...
    public void insertar(EmpleadoBean bean) {
        System.out.println(bean.getClave());
        Connection cn = null;
        try {
    ...
    try {
      ...
      //actualizar registro
      String sql="update empleado set vch_emplpaterno = '"+bean.getPaterno()+
              "', vch_emplmaterno='"+bean.getMaterno()+"',"+
              "vch_emplnombre='"+bean.getNombre()+"',"
              + "vch_emplciudad='"+bean.getCiudad()+"',vch_empldireccion='"+bean.getDireccion()+"',"
              + "vch_emplusuario='"+bean.getUsuario()+"',vch_emplclave = '"+bean.getClave()+"' "+
        "where chr_emplcodigo='"+bean.getCodigo()+"'";
      PreparedStatement pstm = cn.prepareStatement(sql);
      pstm.executeUpdate();
      pstm.close();
      // Confimar transacciÃ³n
