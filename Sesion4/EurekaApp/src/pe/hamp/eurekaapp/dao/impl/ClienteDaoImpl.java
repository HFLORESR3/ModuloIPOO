...
public class ClienteDaoImpl
  ...
  /**
   * Lee un cliente por cÃ³digo.
   *
   * @param codigo CÃ³digo del cliente a leer.
   ...
   *
   * @param bean Este bean trae los datos por los que se realizarÃ¡ la consulta.
  ...
  public void actualizar(ClienteBean bean) {
    ...
    try {
      ...
      //actualizar registro
      String sql="update cliente set vch_cliepaterno = '"+bean.getPaterno()+
              "', vch_cliematerno='"+bean.getMaterno()+"',"+
              "vch_clienombre='"+bean.getNombre()+"',chr_cliedni='"+bean.getDni()+"',"
              + "vch_clieciudad='"+bean.getCiudad()+"',vch_cliedireccion='"+bean.getDireccion()+"',"
              + "vch_clietelefono='"+bean.getTelefono()+"',vch_clieemail = '"+bean.getEmail()+"' "+
        "where chr_cliecodigo='"+bean.getCodigo()+"'";
      PreparedStatement pstm = cn.prepareStatement(sql);
      pstm.executeUpdate();
      pstm.close();
      // Confimar transacciÃ³n
   ...
   * @throws SQLException En caso de algun error, se genera una excepciÃ³n de tipo SQLException.
   */
  @Override
